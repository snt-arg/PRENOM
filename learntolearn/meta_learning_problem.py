...
class MultiObjectiveMixedMetaLearn(ElementwiseProblem):
    def __init__(self, **kwargs):
        ...
        if not SUBSAMPLED_STATES.get(self.category):
            print("Category not found in SUBSAMPLED_STATES. Using default settings.")
        self.subsampled_states = SUBSAMPLED_STATES.get(
            self.category, SUBSAMPLED_STATES["default"]
        )
        ...
        vars = {
            ...
            "density_lambda": Real(
                bounds=tuple(self.subsampled_states["density_lambda"])
            ),
            ...
            "log2_hashmap_size": Choice(
                options=tuple(self.subsampled_states["log2_hashmap_size"])
            ),
            "per_level_scale": Real(
                bounds=tuple(self.subsampled_states["per_level_scale"])
            ),
            "n_neurons": Choice(options=list(self.subsampled_states["n_neurons"])),
            "n_hidden_layers": Choice(
                options=list(self.subsampled_states["n_hidden_layers"])
            ),
        }
        ...
        out["F"] = list(
            evaluate_run(
                ...
                inner_loops_to_test=[
                    ...
                    500,
                    1000,
                ],  # multiple of 50, otherwise will cut
                use_depths=[
                    True,
                ],
