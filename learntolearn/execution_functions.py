...
def run_single_meta_iteration(
    ...
    for i in range(num_meta_loops - 1):
        ...
        if i == num_meta_loops - 2 and save_prior:
            ...
            shutil.copyfile(
                os.path.join(output_dir, f"meta_{identifier}.ply"),
                os.path.join(prior_dir, "model.ply"),
            )
            shutil.copyfile(
                os.path.join(output_dir, f"{identifier}_density.ply"),
                os.path.join(prior_dir, "density.ply"),
            )
            shutil.copyfile(
                os.path.join(output_dir, f"meta_{identifier}.json"),
                os.path.join(prior_dir, "weights.json"),
            )
            ...
            with open(f"{data_dir}/obj_offline/0.txt", "r") as f:
                ...
                lines = f.readlines()[1:]
                _, _, _, _, _, _, _, a1, a2, a3, a4, a5, a6 = [
                    float(x) for x in lines[0].split()[1:]
                ]
...
def evaluate_run(
            ...
            for test_set in test_sets:
                ...
                for line in lines:
                    if "train_time: " in line:
                        total_time += float(
                            line.partition("train_time: ")[2].partition(" ")[0]
                        )
                ...
                with open(
                    os.path.join(test_dir, test_set, "obj_offline/scale.txt"), "r"
                ) as file:
    ...
    second_objective = (
        LAMBDA_MODEL_SIZE * model_size + LAMBDA_TRAIN_TIME * time_per_iteration
    )
